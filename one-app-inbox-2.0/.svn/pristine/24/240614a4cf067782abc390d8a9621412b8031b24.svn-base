...
function showOverLayPopup() {
    ...
    // preview pane setup
    $('.stopPropagation > li').click(function(e){
        var url = $(this).find('.more').attr('href');
        if (!$(this).hasClass('current')) {
            $('.preview-pane .preview').animate({left: "-435px"}, 300, function(){
                $(this).animate({left: "-22px"}, 500).html('<img src="../images/ajax-loader.gif" style="margin: auto; position: absolute; top: 0pt; bottom: 0pt; left: 0pt; right: 0pt;"/>').load(url);
            });
        } else {
            $('.preview-pane .preview').animate({left: "-435px"}, 300);
        }
