...
<html>
    <head>
        ...
        <script type="text/javascript">
			...
			function submitForm(ele){
				...
				parentNode.action = parentNode.action.replace("index","save");
				$(parentNode).submit();
			}
	...
	<body>
		...
		<div class="main-content">
			<header>
				<h2><g:message code="default.new.create.label" args="[formName]" /></h2>
			...
			<section class="container_6 clearfix">
					...
					<g:form useToken="true" enctype="multipart/form-data">
						<div class="tabbed-pane">
                            ...
                            <tr class="prop" id="${p.name}ControlHolder" ${(!display && p.type == 'FormulaField' && !showFields)?'style="display:none;"':''}>
                                ...
                                <td valign="top" class="value ${fieldsNeedingPadding.contains(field?.type)?'labelValue'+(fieldsNeedingPaddingTop.contains(field?.type)?'Link':''):''} ${(domainInstance.errors?.name?.contains(p.name))?'errors':''}" 
									...
									/></g:elseif><g:elseif test="${field?.type == 'SingleLineDate'}"><g:if test="${settings.showCalendar}"><div id="singleDateField"><input 
										type="hidden" id="${p.name}Value" disabled="disabled" itsType="SingleLineDate" value="${domainInstance.getAt(p.name)?(sdf.format(domainInstance.getAt(p.name))):val}" 
				...
				></table
									...
									><g:elseif test="${field?.type == 'SubForm'}"
										><div class="tableContainer" style="width:520px; overflow: auto; border: none " ><g:set var="subFormInstance" value="${null}" 
										/><g:set var="subFormFieldTypes" value="${grailsApplication.config.subFieldTypes}" 
										/><g:if test="${settings.subForm}"><%
											try{
												subFormInstance = Form.read(settings.subForm)
											}catch(Exception e){}
										%></g:if
										><g:if test="${subFormInstance && domainInstance}"
											><table class="datatable" cellpadding="0" cellspacing="0" style="margin:0;">
												<thead>
													<tr>
														<th style="text-decoration:none;font-weight:bold;">Edit</th>
														<g:set var="fieldsList1" value="${subFormInstance.fieldsList.findAll{(subFormFieldTypes.contains(it.type))}}" />
														<g:each in="${fieldsList1}" var="subField" status="j">
															<g:set var="fieldTitle" value="${subField.toString()}" />
															<th style="font-weight:bold;">${fieldTitle}</th>
														</g:each>
													</tr>
												</thead>
											</table
											...
											<td colspan=2>
								                ...
								                <div class="action">
										             ...
										             <g:if test="${!fieldsList.findAll{it.type.contains('Paypal')}}">
											     	 <g:actionSubmit class="button button-green" action="save" style="width: 90px" value="${message(code: 'default.button.create.label', default: 'Create')}"  />
