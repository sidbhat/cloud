...
function handleRuleOn(element){
	...
	}catch(e){alert("error in handleRuleOn function"+e)}
}
...
function checkPageSkipLogic(submittedPageB,isNext){
	if(pageRules){
			...
			for(var i=0;i<pageRules.length;i++){
				...
				if(conditionResult){
					...
					if(pageRules[i].errorMsg && pageRules[i].errorMsg!=''){
						var ruleErrorsEle = $('#ruleErrors')
						ruleErrorsEle.html('<ul><li></li></ul>');
						$('li',ruleErrorsEle).text(pageRules[i].errorMsg);
						ruleErrorsEle.show();
					}else{
						$('#ruleErrors').hide();
					}
		...
		}catch(e){alert("Error while checking page rules")}
...
function checkCondition(fieldRule){
	...
	for(var j=0;j<itsConditions.length;j++){
		...
		}catch(e){alert("Error while getting value of element with id: "+fieldId+", and fieldType: "+fieldType)}
