...
class InputTagLib {
    ...
    def autoComplete = {attrs ->
        ...
        def typeAhead = attrs.remove('typeAhead')
        def typeAheadDelay = attrs.remove('typeAheadDelay')
        ...
        <div class="yui-skin-sam yui-ac">
            ...
            <input type="text" class="yui-ac-input" id="${id}" name="${name}" value="${value}"/>
        ...
        if (options) {
            out << """
                        var dataSource = new YAHOO.util.LocalDataSource([${grailsUITagLibService.collectionToConfig options}]);
        ...
        out << """
                    ...
                    GRAILSUI.${jsid}.typeAhead = ${typeAhead};
                    GRAILSUI.${jsid}.typeAheadDelay = ${typeAheadDelay};
...
"""
    ...
    def datePicker = {attrs ->
        ...
        def id = attrs.remove('id')
        def close = attrs.remove('close')
        def showButton = attrs.remove('hideButton')
        ...
        <script>
            YAHOO.util.Event.onDOMReady(function(){
               ...
               GRAILSUI.${jsid}.selectEvent.subscribe(${jsid}SelectHandler);
                ...
                out << """YAHOO.util.Event.on("${id}", "focus", function() {
                    var buttonRegion = YAHOO.util.Dom.getRegion('${id}');
                    var buttonHeight = buttonRegion.bottom - buttonRegion.top;
                    var buttonWidth = buttonRegion.right - buttonRegion.left;
