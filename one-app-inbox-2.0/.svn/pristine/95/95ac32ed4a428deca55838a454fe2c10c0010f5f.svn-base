...
(function($, window, document) {
	...
	$.fn.dataTable = function( oInit )
	{
		...
		function _fnFilterColumn ( oSettings, sInput, iColumn, bRegex, bSmart )
		{
			...
			var rpSearch = _fnFilterCreateSearch( sInput, bRegex, bSmart, oSettings.searchAnyWord);
		 ...
		 *           bool:bRegex - treat as a regular expression or not
		 *           bool:bSmart - perform smart filtering or not
		 *           bool:searchAnyWord - used to make condition AND(if false) or OR(if true)
		 */
		function _fnFilterCreateSearch( sSearch, bRegex, bSmart, searchAnyWord )
		{
			...
			if ( bSmart )
			{
				...
				var asSearchNew = new Array();
				for(var i=0;i<asSearch.length;i++){
					if(asSearch[i]!=""){
						asSearchNew[asSearchNew.length]=asSearch[i];
					}
				}
				asSearch=asSearchNew;
				sRegExpString = '^(?=.*?'+asSearch.join( ')'+(searchAnyWord?'|':'')+'(?=.*?' )+').*$';
		...
		return this.each(function()
		{
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "searchAnyWord", "searchAnyWord" );
