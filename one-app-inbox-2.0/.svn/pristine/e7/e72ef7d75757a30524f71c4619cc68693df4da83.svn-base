...
class AttachmentsTagLib {
    ...
    def deleteLink = {attrs, body ->
		...
		def params = [:]
		if (returnPageURI) {
			params.returnPageURI = returnPageURI
		}
		if (attrs.feed) {
			params.feed = attrs.feed
		}
		if(params) {
			attrs.params = params
		}
    ...
    def downloadLink = {attrs, body ->
        ...
        attrs.id = attachment?.id?:attrs.attachmentId
        def params = [:]
    ...
    static final Map IMAGE_ICON_MAP = [
			gif: 'page_white_picture',
			png: 'page_white_picture',
			jpg: 'page_white_picture',
			jpeg: 'page_white_picture',
    ...
    def icon = {attrs ->
		...
		def outString = ""
		if(bean.ext && (IMAGE_ICON_MAP.keySet() as List).contains(bean.ext.toLowerCase())){
			def paramsMap = [showImg:bean.id]
			if(attrs.feed){
				paramsMap.feed = attrs.feed
			}
			outString = "<div style=\"display:inline-block;border:1px solid #DDDDDD;height:100px;left:0;margin:auto;overflow:hidden;position:relative;right:0;width:90px;\"><a class=\"help\" href=\""+createLink(controller:'layouts',action:'image_pop.gsp',params:paramsMap)+"\" rel=\"#overlay\"><img class=\"attachmentImg\" style=\"max-height: 100px\" src=\"" +createLink(controller:'attachmentable',action:'renderImg',id:bean.id)+"\" alt=\"${bean.ext}\"/></a></div>"
		}else{
			outString = "<img width=\"16\" height=\"16\" src=\"" + g.resource(plugin:'attachmentable', dir: 'images/silk', file: FILE_ICON_MAP[bean.ext?.toLowerCase()] ?: 'page_white') + ".png\" alt=\"${bean.ext}\"/>"
		}
        out << outString
    }
	
	def imagePopUp = {attrs ->
		def attachmentId = attrs.attachmentId
		out << """<img align="middle" id="mainImg" style=" max-width: 500px"src="${createLink(controller:'attachmentable',action:'renderImg',id:attachmentId)}" onload="checkImgSize(this);">"""
	}
