...
class MailService {
    ...
    MailMessage sendMail(Closure callable,def mailConfigurations = null) {
		if (isDisabled()) {
			log.warn("Sending emails disabled by configuration option")
		} else {
			def messageBuilder = mailMessageBuilderFactory.createBuilder(mailConfigurations?:mailConfig)
