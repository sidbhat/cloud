...
class Phone extends Widget {
 ...
 private String widgetTemplateText(String name, Object settings, 
    ...
    String textField = ""
	  ...
	  if(!formDesignerView){
		  try{
			String stringValue = domainInstance."${name}"
			stringValueScript = '<script>$("#'+name+'").val("'+(stringValue?stringValue.encodeAsJavaScript():'')+'")</script>'
		  }catch(Exception e){}
	  }
//	  PhoneNumberUtil phoneUtil = PhoneNumberUtil.getInstance();
//	  PhoneNumber usNumberProto = phoneUtil.getExampleNumber(settings.format=="ZZ"?"US":settings.format)
//	  def ex = phoneUtil.format(usNumberProto, settings.format=="ZZ"?PhoneNumberFormat.INTERNATIONAL:PhoneNumberFormat.NATIONAL)
	  
	  if (!readOnly) {
		  textField += """<input type="text" style="${isEmailEmbedFrom?'width:50%;':''}${fontFamily}font-weight: ${fontWeight}; font-style: ${fontStyle}; \
				text-decoration: ${textDecoration}; ${fontSize}${valueColor}${valueBackgroundColor}" name="${name}" id="${name}" class="textInput" value="${value?:''}"/>"""
//		  placeholder="${ex?:''}"
