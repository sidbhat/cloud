 ...
 * @since 0.1
 */
class MultiLineText extends Widget {
 ...
 private String widgetTemplateText(String name, Object settings, 
    ...
    String textField
	def fieldSize=settings.fieldSize?settings.fieldSize.toInteger():2
	  String stringValue = formDesignerView ? (""+settings."${language}".value) : (""+(domainInstance?."${name}"?:''))
	  ...
	  if (!readOnly) {
		  textField = """\
		  <textarea name="${name}" id="${name}" class="textInput"
		    style="${isEmailEmbedFrom?'width:50%;height:75px;':''}${fontFamily}font-weight: ${fontWeight}; font-style: ${fontStyle}; \
		    text-decoration: ${textDecoration}; ${fontSize}${valueColor}${valueBackgroundColor}; height: ${fieldSize*1*65+'px'}; resize: none;" ></textarea>
		  """+stringValueScript
	  } else {
		  textField = """\
	  ...
	  return """<div><div 
		  		...
		  		style="text-decoration: ${textDecoration};">${settings."${language}".label.encodeAsHTML()}</span><em>&nbsp;${settings.required ? '*' : FormBuilderConstants.EMPTY_STRING}</em></label></div><div 
