...
class LookUp extends Widget {
 ...
 private String widgetTemplateText(String name, Object settings, 
    ...
    String textField
	  ...
	  if (!readOnly) {
		  textField = """\
		  <input type="text" name="${name}" class="lookUpInput" id="${name}" class="textInput"
		  ...
		  /><div class="LookUpButton" id="${name}search"></div>"""+stringValueScript
	  } else {
		  textField = """\
	  ...
	  return """<div><div 
				...
				class="formHint LookUpTypeFieldHint" style="${descriptionColor}${descriptionBackgroundColor}">${settings."${language}".description.encodeAsHTML()}</p></div></div>"""
  }
