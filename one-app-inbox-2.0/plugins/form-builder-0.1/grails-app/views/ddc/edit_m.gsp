     ...
     <style type="text/css">
	             ...
	             <p style="text-align:center;">
					<g:each in="${domainInstance.errors }" var="error">
						<g:if test="${error.name != 'version'}">
		                	<g:set var="fieldWithSettings" value="${fields?.get(error.name.toString()) }" />
		                	<g:if test="${fieldWithSettings }">
			               	    <g:set var="fieldName" value = "${grails.converters.JSON.parse(fieldWithSettings.settings)?.en?.label}" />
		...
		<div data-role="header" data-position="fixed">
			<g:if test="${form.formCat == 'S'}">
				<a data-icon="back" class="ui-btn-left" href="${request.getContextPath()}/ddc/edit/${params.pfii}?dc=${params.pdc}" >Back</a>
			</g:if>
	        <g:else>
                       ...
                       <g:elseif test="${display || showFields || p.type == 'FormulaField'}">
	                             ...
	                             <g:elseif test="${field?.type == 'LookUp' }"><g:textField 
									...
									/></g:elseif><g:elseif test="${field?.type == 'SingleLineDate'}"><g:if test="${settings.showCalendar}">
									 <input type="date" name="${p.name}" value="${domainInstance.getAt(p.name)?(sdf.format(domainInstance.getAt(p.name))):''}" itsType="SingleLineDate"  data-role="datebox" data-options='{"mode": "flipbox","dateFormat": "MM/DD/YYYY"}'
									  /></g:if><g:else><div id="boxDateField"><input type="text" style="width:30px !important;display:inline-block !important;" placeholder="MM" id="${p.name}dateMM" onchange="setFieldValue${p.name}()"
						...
						></g:else
									...
									><g:elseif test="${field?.type == 'SubForm'}"
										><form:drawSubFormMobile settings="${settings}" isEditable="${isEditable}" showFields="${showFields}" domainInstance="${domainInstance}" field="${field}" pfid="${form.id}" style="width:520px;" 
                        ...
                        <g:if test="${pageBreakFields}">
		                     <div data-role="fieldcontain" style="padding:0.4em 15px;border:none;"><div 
								...
								class="nextButton" style="width:49%;display:inline-block;padding-top: 8px;"><input type="hidden" name="last" class="button button-gray" value="Next" style="display:none;" /> Page ${pageBreakCount++} of ${pageBreakFields.size()+1}</div></div>
			            </g:if>
