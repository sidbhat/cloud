...
<html>
	...
	<% def groupName %>
		...
		<g:if test="${domainInstance.errors }">
			...
			<g:uploadForm action="update" data-ajax="false">
				...
				<label for="UniqueId" class="ui-input-text">Unique Id</label><input type="text"  value="${valueOfField}"  disabled="disabled" readonly="readonly" />
                             ...
                             /></g:else></g:elseif>
	                             ...
	                             <g:elseif test="${field?.type == 'LookUp' }"><g:textField 
									...
									itsAttachments?.each{attachment->
											attachmentText += ('<br/><label class="ui-input-text" ></label ><a href="'+grailsApplication.config.grails.serverURL+'/attachmentable/download/'+attachment.id+'" target="_blank">'+attachment.name.encodeAsHTML()+'</a>')
											attachmentText += " - "
											def returnPageURI = grailsApplication.config.grails.serverURL+"/"+params.controller+"/edit/"+params.id+"?formId="+params.formId+((params.pfid && params.pfii && params.pffn)?"&pfid=${params.pfid}&pfii=${params.pfii}&pffn=${params.pffn}":'')
											attachmentText += ('<a href="'+grailsApplication.config.grails.serverURL+'/attachmentable/delete/'+attachment.id+'?returnPageURI='+returnPageURI.encodeAsURL()+'">Delete</a>')
										   attachmentText += '<br/>'
									}
									 %>
									 ${attachmentText}
									...
									%><div class="addressControl"><label for="${p.name}_city" class="ui-input-text"> Address Line 1</label><input type="text"  value="${mapValue?.'line1'}" name="${p.name}_line1"  id="${p.name}_line1" class="addressInput"
			    						...
			    						></div
		    						></g:elseif><g:elseif test="${field?.type == 'NameTypeField'}"
										><%
				   ...
				   <input type="hidden" name="subFormfn" id="subFormfn" value="null" />
					...
					<g:if test="${!fieldsList.findAll{it.type.contains('Paypal')}}">
						<g:submitButton name="update"  data-icon="check" value="${message(code: 'default.button.update.label', default: 'Update')}" />
					</g:if>
				</div>
			</g:uploadForm>
			</g:if>
