...
<script type="text/javascript" src="${resource(dir: 'js', file: 'formula.js')}"></script>
		...
		<div data-role="header" data-position="fixed">
			<g:if test="${formInstance.formCat == 'S'}">
				<a data-icon="back" class="ui-btn-left" href="${request.getContextPath()}/PF/edit/${params.pfii}?formId=${params.pfid}" >Back</a>
			</g:if>
		...
		<div data-role="content">
			<%org.grails.formbuilder.Form form  = formInstance %>
			...
			<g:uploadForm action="save" ${ multiPart ? ' enctype="multipart/form-data"' : '' } data-ajax="false">
				...
				<g:each in="${fieldsList}" var="p" status="i">
	                ...
	                <g:if test="${settings.hideFromUser==true}"><sec:ifNotLoggedIn><g:set var="display" value="${false }" /></sec:ifNotLoggedIn></g:if>
                             ...
                             <g:elseif test="${field?.type == 'LookUp' }"><g:textField 
									...
									></g:elseif><g:elseif test="${field?.type == 'ImageUpload'}"
										> &nbsp;<%
											...
											></g:if>
                                        </g:elseif><g:elseif test="${field?.type == 'SubForm'}"
