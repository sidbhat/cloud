...
class AttachmentableService {
    ...
    List<MultipartFile> upload(def poster,
                               def reference,
                               List<MultipartFile> files,Long domainInstanceId) {
        def uploadedAttachment = []

        try {
            Attachment.withTransaction {status ->
                files.each {MultipartFile file ->
                    try {
                        def attachment = addAttachment(poster, reference, file,domainInstanceId)
    ...
    def addAttachment(def poster, def reference, CommonsMultipartFile file,def domainInstanceId) {
        addAttachment(CH.config, poster, reference, file,domainInstanceId)
    }

    def addAttachment(def config,
                      def poster,
                      def reference,
                      CommonsMultipartFile file, Long domainInstanceId) {
	 ...
	 def addEmailAttachment(def config,
						  def poster,
						  def reference,
						   String filename,
						  InputStream input,
			...
			File diskFile = saveFile(config, attachment, input)
			attachment.length = diskFile.length()
			if(reference.respondsTo('onAddAttachment')) {
				reference.onAddAttachment(attachment)
			}
	...
	def saveFile(def config, Attachment attachment,InputStream input) throws IOException {
			 
		
		File uploadDir = AttachmentableUtil.getDir(config, attachment, false)
    ...
    boolean removeAttachmentWithId(Long attachmentId) {
    ...
    PagedResultList findAttachmentsByReference(def reference,
                                               List inputs,
                                               def params = [:], def domainInstanceId = null) {
        if (!reference) {
            throw new AttachmentableException(
                    "Reference is null.")
        }
        ...
        PagedResultList result = Attachment.createCriteria().list(params) {
			...
			if(domainInstanceId){
				eq 'domainInstanceId',"${domainInstanceId}".toLong()
			}
            lnk {
                eq 'referenceClass', reference.class.name
                eq 'referenceId', reference.ident()
