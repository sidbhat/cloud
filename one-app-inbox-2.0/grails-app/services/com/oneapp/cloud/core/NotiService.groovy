...
class NotiService {
				...
				if(sendFeed && formAdmin.formSubmissionAction=='Feed' && (action=='create' && formAdmin.notiOnCreate)){
					if(currentUser && currentUser?.userTenantId>0){
						...
						TenantUtils.doWithTenant(formAdmin.form.tenantId){
							createFeed(domainInstance,formAdmin,User.findByUsername("publicuser@yourdomain.com"))
						}
					}
					...
					if(currentUser && currentUser?.userTenantId>0){
						...
						TenantUtils.doWithTenant(formAdmin.form.tenantId){
							sendMail(domainInstance,formAdmin,User.findByUsername("publicuser@yourdomain.com"),uniqueId,action,sendFeed)
						}
					}
				}
	...
	def sendMail(def domainInstance,FormAdmin formAdmin,User currentUser,def uniqueId,def action,def sendFeed){
		...
		def templateText = formEmailBody(action,formName,
			g.render(template:'/activityFeed/formDetailsEmail', model:[notiForm:formAdmin.form,notiDI:domainInstance]), uniqueId,currentUser)
		if(sendFeed){//Notification email to internal users: will only be sent when the special conditions are passed
			if((action=='create' && formAdmin.notiOnCreate) || (action=='update' && formAdmin.notiOnUpdate)){
				formAdmin.formSubmissionTo.each{usr->
					...
					async.emailData=templateText.replace("<@user>",usr?.firstName)
					//"<span style=\"font-family: 'Helvetica Neue',Helvetica,Helvetica,Arial,sans-serif;vertical-align:top;color:#455560;font-weight:700;margin:0;font-size: 14px\">Dear ${usr?.firstName},<br><br>You have ${action=='create'?'a new form submission':'an entry updated'} for your form <b>${formName}</b> by: <b>${currentUser.firstName?:''} ${currentUser.lastName?:''}</b>${currentUser.userTenantId!=0?('&lt;'+currentUser.username+'&gt;'):''}<br><br>"+templateText+"<br><br>${uniqueId?('You can search this entry with its unique-id: '+uniqueId+'<br>'+'Click <a href=\"'+ConfigurationHolder.config.grails.serverURL+'/PF/publicSearch\">here</a> to search.<br><br>'):''}Thank you,<br>Your Form Builder Team!</span>"
					async.isHtml=true
				...
				if(formEmailField){
					...
					if(emailTo){
						...
						if(matcher.matches()){
							...
							async.emailSubject="${action=='create'?'New Entry':'Entry Updated'} for ${formName} ${uniqueId?('(Unique ID: '+uniqueId+')'):''}"
							async.emailData=templateText.replace("<@user>","User")
							//"Dear User,<br><br>You have ${action=='create'?'a new form submission':'an entry updated'} for form <b>${formName}</b> by: <b>${currentUser.firstName?:''} ${currentUser.lastName?:''}</b>${currentUser.userTenantId!=0?('&lt;'+currentUser.username+'&gt;'):''}<br><br>"+templateText+"<br><br>${uniqueId?('You can search this entry with its unique-id: '+uniqueId+'<br>'+'Click <a href=\"'+ConfigurationHolder.config.grails.serverURL+'/PF/publicSearch\">here</a> to search.<br><br>'):''}Thank you,<br>Your Form Builder Team!"
							async.isHtml=true
							async.emailSentStatus = AsynchronousEmailStorage.NOT_ATTEMPT
							async.save()
						}else{
	...
	def  formEmailBody(String action,String formName,def templateText,String uniqueId, def currentUser){
		String UUIDsearch=uniqueId?('You can search this entry with its unique-id: '+uniqueId+'<br>'+
			'Click <a href="'+ConfigurationHolder.config.grails.serverURL+'/PF/publicSearch">here</a> to search.<br><br>'):''
		return """<span 
				style="font-family: 'Helvetica Neue',Helvetica,Helvetica,Arial,sans-serif;vertical-align:top;color:#455560;
				font-weight:700;margin:0;font-size: 14px"
		  >Dear <@user>,<br><br>You have ${action=='create'?'a new form submission':'an entry updated'} for your form <b>${formName}</b> by: <b
		  >${currentUser.firstName?:''} ${currentUser.lastName?:''}</b>${currentUser.userTenantId!=0?('&lt;'+currentUser.username+'&gt;'):''}<br><br
		  >${templateText}<br><br>${UUIDsearch}Thank you,<br>Your Form Builder Team!</span>""" 		
		}
}
